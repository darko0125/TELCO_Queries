...
not_found AS (
  SELECT 
    cast(tgbr.parent_call_sid AS string) AS call_id,
    tgbr.direction AS call_type,
    tgbr.status AS call_status,
    tgbr.from AS caller_number,
    -- Mapping for dialed_number
    CAST(
      CASE 
        when tgbr.forwarded_from is not null then REGEXP_REPLACE(cast(`forwarded_from` as string), r'^1|^55|^81|^44|^52|^61|^64|^65|^31|^49|^43|^32|^353|^56|^54','') 
    ...
    tgbr.caller_name AS caller_name,
    NULL AS caller_city,
    ...
    TIMESTAMP(tgbr.start_time) AS start_time,
    CAST(tgbr.duration AS STRING) AS duration,
    CAST(CURRENT_TIME() AS STRING) AS created_on,
    CAST(CURRENT_TIME() AS STRING) AS updated_on,
    tgbr.mailbox_url AS mailbox_url,
    NULL AS call_page_info,
    NULL AS call_page_geo_keyword_id
  FROM `dms-data-lake-development.twilio_gbr.calls_report` tgbr
  LEFT JOIN `localiq-dms-analytics-v2-prod.rl_cts.DniGroup` dg
    ON tgbr.group_sid = CAST(dg.publisher_group_id AS STRING)
  WHERE DATE(start_time) > '2020-01-01'
    AND (dg.idDniGroup IS NULL OR tgbr.group_sid IS NULL)
)
SELECT 
  n.call_id,
  n.call_type,
